...
export function hasLiveProductTag(product, liveTag) {
  if (!liveTag) return true;

  const needle = liveTag.toLowerCase();
  return (product.tags || []).some((tag) => String(tag).toLowerCase() === needle);
}

export async function fetchPrintifyProducts({ token, shopId, liveTag }) {
  if (!token) {
...
export async function fetchPrintifyProduct({ token, shopId, productId, liveTag }) {
  if (!token) {
    throw new Error('Missing PRINTIFY_TOKEN');
  }
