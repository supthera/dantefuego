...
export async function handleStripeWebhook(env, rawBody, signatureHeader, db) {
  ...
  const fulfillment = session.metadata?.fulfillment || 'printify';
  let printifyOrderId = null;

  if (fulfillment === 'printify') {
    const result = await submitPrintifyOrder(env, { session, lineItems });
    printifyOrderId = result.printify_order_id;
  } else if (fulfillment === 'manual') {
  ...
  const order = {
    ...
    status: fulfillment === 'printify' ? 'submitted_to_printify' : 'manual_pending',
    ...
    line_items_json: JSON.stringify(
      printifyOrderId ? lineItems.map((item) => ({ ...item, printify_order_id: printifyOrderId })) : lineItems
    ),
