...
export async function getHealthStatus({ token, shopId, liveTag, hiddenProductIds }) {
  const status = {
    ok: false,
  ...
  try {
    const products = await fetchPrintifyProducts({ token, shopId, liveTag, hiddenProductIds });
    status.ok = true;
