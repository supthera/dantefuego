...
export async function createCheckoutSession(env, payload, request) {
  ...
  params.set(
    'return_url',
    `${siteUrl}/checkout.html?checkout=complete&session_id={CHECKOUT_SESSION_ID}`
  );
  params.set('shipping_address_collection[allowed_countries][0]', 'US');
  params.set('customer_creation', 'always');
  params.set('invoice_creation[enabled]', 'true');
  params.set('branding_settings[display_name]', 'Dante Fuego');
  params.set('branding_settings[background_color]', '#000000');
  params.set('branding_settings[button_color]', '#8b0000');
  params.set('branding_settings[border_style]', 'rounded');
  params.set('branding_settings[font_family]', 'roboto');
...
export async function getCheckoutSessionStatus(env, sessionId) {
  ...
  const session = await response.json();
  if (!response.ok) {
    throw new Error(session.error?.message || 'Failed to retrieve checkout session');
  }
